...
IMAGE="${SMOKE_IMAGE:-ndm-release-v02-public-hf-smoke:20260529}"
DOCKERFILE="${SMOKE_DOCKERFILE:-$ROOT/docker/release-v02-public-hf-smoke.Dockerfile}"
OUTPUT_DIR="${SMOKE_OUTPUT_DIR:-/tmp/release-v02-docker-public-hf-smoke-${USER:-agent}}"
...
REVISION="${SMOKE_REVISION:-v0.2}"

: "${SMOKE_E88_EXPECTED_SHA:?SMOKE_E88_EXPECTED_SHA is required}"
: "${SMOKE_GDN_EXPECTED_SHA:?SMOKE_GDN_EXPECTED_SHA is required}"
: "${SMOKE_M2RNN_EXPECTED_SHA:?SMOKE_M2RNN_EXPECTED_SHA is required}"
...
expected_sha() {
  case "$1" in
    e88) printf '%s\n' "$SMOKE_E88_EXPECTED_SHA" ;;
    gdn) printf '%s\n' "$SMOKE_GDN_EXPECTED_SHA" ;;
    m2rnn) printf '%s\n' "$SMOKE_M2RNN_EXPECTED_SHA" ;;
...
run_model() {
  ...
  local args=(
    --model "$model"
    --revision "$REVISION"
    --expected-sha "$(expected_sha "$model")"
    --device "$device"
    --prompt "$PROMPT"
...
cpu_cache="ndm-public-hf-v02-smoke-cpu-$suffix"
gpu_cache="ndm-public-hf-v02-smoke-gpu-$suffix"
...
for path in sorted(output_dir.glob("*.json")):
    ...
    row = {
        ...
        "release_revision_name": data.get("release_revision_name"),
        ...
        "param_count_ok": data.get("param_count_ok"),
    ...
    ok = ok and row["expected_sha"] == row["resolved_sha"]
    ok = ok and row["private"] is False
