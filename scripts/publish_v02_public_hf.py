#!/usr/bin/env python3
"""Publish validated v0.2 artifacts to the public Hugging Face model repos.

This script is intentionally narrow and approval-gated. It uploads only the
files already listed in the local v0.2 validation manifest, verifies readback at
the uploaded commit before tagging, and never deletes or recreates existing
v0.1 tags.
"""
...
DEFAULT_WORKDIR = Path("/tmp/release-v02-local-hf-candidates-agent-500")
DEFAULT_LOCAL_SMOKE = Path("/tmp/release-v02-docker-local-hf-artifact-smoke-agent-500/summary.json")
DEFAULT_OUTPUT = Path(f"/tmp/release-v02-public-hf-publish-{os.environ.get('WG_AGENT_ID', 'agent')}/summary.json")
RELEASE_TAG = "v0.2"
...
V01_RESOLVED_SHAS = {
    "e88": "a2e56cb82eec5e01ae6eb501569359c5ff64af6b",
    "gdn": "556df7f00969c6a8dbeb381e3c8b51cf0c0385f9",
    "m2rnn": "8181b77803e130ffd78e37c33aa4d58c27e719c2",
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if data.get("release_candidate") != "v0.2-rc-local":
...
def ensure_approval(args: argparse.Namespace) -> None:
    ...
    required = [
        "authorizes public hugging face v0.2 publication",
...
def preflight_public_refs(api: HfApi, model: dict[str, Any]) -> dict[str, Any]:
    ...
    v01 = get_public_info(api, repo_id, "v0.1")
    if getattr(v01, "sha", None) != V01_RESOLVED_SHAS[key]:
        raise RuntimeError(f"{repo_id} v0.1 sha changed before upload: {getattr(v01, 'sha', None)}")
    refs = refs_dict(api, repo_id, token=False)
    return {
        "repo_id": repo_id,
        "main_sha": getattr(main, "sha", None),
        "v0_1_sha": getattr(v01, "sha", None),
        "v0_2_target_before": ref_target(refs, "tags", RELEASE_TAG),
...
def upload_and_tag_model(
    ...
    key = model["key"]
    repo_id = model["repo_id"]
    existing_v02 = preflight["v0_2_target_before"]
    if existing_v02:
        task_log(f"{repo_id}: existing v0.2 tag found at {existing_v02}; verifying it matches validated artifacts.")
        emit_event("existing_v02_readback_start", repo_id=repo_id, sha=existing_v02)
        existing_readback = verify_remote_revision(api, model, local, RELEASE_TAG, cache_dir / key / "existing-v02")
        if existing_readback["resolved_sha"] != existing_v02:
            raise RuntimeError(f"{repo_id} v0.2 ref target/readback mismatch")
        task_log(f"{repo_id}: existing v0.2 tag verified at {existing_v02}; v0.1 was not modified.")
        emit_event("existing_v02_readback_finish", repo_id=repo_id, sha=existing_v02)
        return {
            "repo_id": repo_id,
            "action": "existing_v0_2_verified",
            "upload_commit": existing_v02,
            "tag_sha": existing_v02,
            "pre_tag_readback": existing_readback,
        }

    artifact_dir = local["artifact_dir"]
    task_log(f"{repo_id}: starting public v0.2 upload from validated local artifacts in {artifact_dir}.")
    ...
    commit = api.upload_folder(
        ...
        token=True,
        commit_message=f"Upload public v0.2 artifacts for {MODEL_LABELS[key]}",
        commit_description=(
            f"Approved public v0.2 upload from validated local artifacts.\n\n"
            f"Source checkpoint SHA256: {model['checkpoint']['sha256']}\n"
            f"Checkpoint step: {model['checkpoint']['step']}\n"
            "v0.1 tags are intentionally left untouched."
        ),
    ...
    task_log(f"{repo_id}: pre-tag readback verified commit {upload_sha}; creating v0.2 tag.")
    ...
    api.create_tag(
        ...
        tag=RELEASE_TAG,
        revision=upload_sha,
        tag_message=f"v0.2 public release for {MODEL_LABELS[key]}",
        token=True,
        exist_ok=False,
    )
    tagged_info = get_public_info(api, repo_id, RELEASE_TAG)
    if getattr(tagged_info, "sha", None) != upload_sha:
        raise RuntimeError(f"{repo_id} v0.2 tag resolved {getattr(tagged_info, 'sha', None)} != {upload_sha}")

    task_log(f"{repo_id}: created v0.2 tag at {upload_sha}.")
...
def postflight_refs(api: HfApi, model: dict[str, Any]) -> dict[str, Any]:
    key = model["key"]
    repo_id = model["repo_id"]
    v01 = get_public_info(api, repo_id, "v0.1")
    v02 = get_public_info(api, repo_id, RELEASE_TAG)
    if getattr(v01, "sha", None) != V01_RESOLVED_SHAS[key]:
        raise RuntimeError(f"{repo_id} v0.1 sha changed after publish: {getattr(v01, 'sha', None)}")
    return {
        "repo_id": repo_id,
        "v0_1_sha": getattr(v01, "sha", None),
        "v0_2_sha": getattr(v02, "sha", None),
        "v0_1_private": getattr(v01, "private", None),
        "v0_2_private": getattr(v02, "private", None),
        "refs": refs_dict(api, repo_id, token=False),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--approved-public-v02-publication", action="store_true")
...
def main() -> int:
    ...
    task_log("Approval confirmed in task context; public v0.2 HF write path is now permitted.")
    emit_event("approval_confirmed", release_tag=RELEASE_TAG)
    if RELEASE_TAG == "v0.1":
        raise SystemExit("refusing to publish to v0.1")
    ...
    for model in selected_models:
        ...
        postflight = postflight_refs(api, model)
        if postflight["v0_2_sha"] != publish["tag_sha"]:
            raise RuntimeError(f"{model['repo_id']} postflight v0.2 tag mismatch")
        task_log(
            f"{model['repo_id']}: postflight public refs verified; "
            f"v0.2={postflight['v0_2_sha']} and v0.1={postflight['v0_1_sha']}."
        )
            ...
            json.dumps(
                {
                    ...
                    "v0.1": postflight["v0_1_sha"],
                    "v0.2": postflight["v0_2_sha"],
                },
                sort_keys=True,
            ),
            flush=True,
    ...
    summary["ok"] = True
    summary["public_v0_2_shas"] = {row["key"]: row["postflight"]["v0_2_sha"] for row in summary["models"]}
    summary["public_v0_1_shas"] = {row["key"]: row["postflight"]["v0_1_sha"] for row in summary["models"]}
    write_json(args.output, summary)
